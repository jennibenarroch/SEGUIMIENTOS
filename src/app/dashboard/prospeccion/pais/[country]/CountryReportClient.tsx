...
import { Users, CheckCircle2, Clock, AlertTriangle, Circle, Sparkles } from "lucide-react";
...
const SEMAFORO: Record<string, { dot: string; text: string }> = {
...
export default function CountryReportClient({
  countryName,
  prospects,
  error,
}: {
  ...
  const stats = useMemo(() => {
    ...
    for (const p of prospects) {
      if (p.seller) bySeller[p.seller] = (bySeller[p.seller] ?? 0) + 1;
    }

    return { newThisWeek, byHealth, bySeller };
  }, [prospects]);

  if (error) {
    return <div className="text-center py-20 text-slate-500 text-sm">Error al cargar: {error}</div>;
  }
    ...
    <>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          ...
          <p className="text-xs text-slate-500 mt-2">prospectos nuevos esta semana en {countryName}</p>
        </div>
        ...
        {Object.keys(stats.bySeller).length > 0 && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <p className="text-xs text-slate-500 uppercase tracking-wider mb-4">Por vendedor</p>
            <div className="space-y-3">
              {Object.entries(stats.bySeller)
                .sort(([, a], [, b]) => b - a)
                .map(([seller, count]) => (
                  <div key={seller} className="flex items-center gap-3">
                    <span className="text-sm text-slate-300 w-36 truncate">{seller}</span>
        ...
        <div>
          <p className="text-xs text-slate-500 uppercase tracking-wider mb-4">
            {prospects.length === 0 ? "Sin prospectos registrados" : `${prospects.length} prospectos`}
          </p>

          {prospects.length === 0 ? (
            <div className="text-center py-16 text-slate-600 text-sm bg-slate-900 rounded-2xl border border-slate-800">
              No hay prospectos registrados para {countryName} todavía.
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-900 border-b border-slate-800">
                  <tr>
                    ...
                    <th className="px-5 py-3 text-left text-xs text-slate-500 font-medium hidden sm:table-cell">Vendedor</th>
                  ...
                  {prospects.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="px-5 py-3.5 text-slate-300 text-xs hidden sm:table-cell">
                          {p.seller || "—"}
                        </td>
